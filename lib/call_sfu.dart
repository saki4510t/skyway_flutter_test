...
const String _PREF_KEY_ROOM = 'skyway.ROOM_SFU';

class RemotePeer {
  bool _hasRemoteStream = false;
}

class CallSfuScreen extends StatefulWidget {
  CallSfuScreen(this.apiKey, this.domain, {Key key, this.title}) : super(key: key);
  ...
  @override
  _CallSfuScreenState createState() => _CallSfuScreenState();
}

class _CallSfuScreenState extends State<CallSfuScreen> {
  ...
  @override
  void didUpdateWidget(CallSfuScreen oldWidget) {
...
//--------------------------------------------------------------------------------
  ...
  Future<void> _enter(String roomName) async {
    if (isConnected && !isTalking && !_isJoined
      ...
      await _peer.join(roomName, SkywayRoomMode.SFU);
      setState(() {
